...
int readSCS(unsigned char *nDat, int nLen)
{
	...
	for (int i = 0; i < nLen; i++) {
		...
		if (osMessageQueueGet(MotorDataParseQueueHandle, &byte, NULL, timeout_ms) == osOK) {
			nDat[i] = byte;
			received++;
		} else {
...
void rFlushSCS() {
	...
	while (max_attempts-- > 0 && osMessageQueueGet(MotorDataParseQueueHandle, &dummy, NULL, 0) == osOK);
